...
static void printType0 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type0) {
		...
		} else { printf("BIOS Starting Segment: 0x%04hX\n" ,ctx -> Type0 -> bios_starting_segment); }
		...
		if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
			...
			} else { printf("Firmware Characteristics Extension Bytes 1: Not Present\n"); }
		} else if (!compact_output) { printf("Firmware Characteristics Extension Bytes 1: [SMBIOS 2.1 required]\n"); }
		if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,3)) {
			...
			} else { printf("Firmware Characteristics Extension Bytes 2: Not Present\n"); }
		} else if (!compact_output) { printf("Firmware Characteristics Extension Bytes 2: [SMBIOS 2.3 required]\n"); }
		if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,4)) {
			...
			} else { printf("Platform Major Release: %hhu\n" ,ctx -> Type0 -> platform_major_release); }
		...
		if (LAZYBIOS_FIELD_STATUS(ctx->Type0 ,extended_rom_size) == LAZYBIOS_FIELD_PRESENT) {
			printf("Extended ROM Size: %hu%s\n\n" ,
			       lazybiosType0ExtendedROMSizeU16(ctx -> Type0 -> extended_rom_size ,ctx -> Type0 -> unit) ,
			       ctx -> Type0 -> unit);
		} else if (LAZYBIOS_FIELD_STATUS(ctx->Type0 ,rom_size) == LAZYBIOS_FIELD_PRESENT) {
			printf("ROM Size: %u KB\n\n" ,ctx -> Type0 -> rom_size);
		} else { printf("ROM Size: Not Present\n\n"); }
	} else { printf("Failed to get BIOS information\n\n"); }
}
...
static void printType1 (lazybiosCTX_t *ctx) {
		...
		if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
			if (LAZYBIOS_FIELD_STATUS(ctx->Type1 ,uuid) == LAZYBIOS_FIELD_PRESENT) {
				printf("UUID: %02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X\n" ,
				       ctx -> Type1 -> uuid[3] ,ctx -> Type1 -> uuid[2] ,ctx -> Type1 -> uuid[1] ,
				       ctx -> Type1 -> uuid[0] ,ctx -> Type1 -> uuid[5] ,ctx -> Type1 -> uuid[4] ,
				       ctx -> Type1 -> uuid[7] ,ctx -> Type1 -> uuid[6] ,ctx -> Type1 -> uuid[8] ,
				       ctx -> Type1 -> uuid[9] ,ctx -> Type1 -> uuid[10] ,ctx -> Type1 -> uuid[11] ,
				       ctx -> Type1 -> uuid[12] ,ctx -> Type1 -> uuid[13] ,ctx -> Type1 -> uuid[14] ,
				       ctx -> Type1 -> uuid[15]);
			} else { printf("UUID: Not Present\n"); }
			if (LAZYBIOS_FIELD_STATUS(ctx->Type1 ,wake_up_type) != LAZYBIOS_FIELD_PRESENT) {
				printf("Wake up type: Not Present\n");
			} else { printf("Wake up type: %s\n" ,lazybiosType1WakeupTypeStr(ctx -> Type1 -> wake_up_type)); }
		} else {
			if (!compact_output)
	...
	} else { printf("Failed to get System information\n\n"); }
}
...
static void printType2 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type2 && ctx -> type2_count > 0) {
		for (size_t i = 0; i < ctx -> type2_count; i++) {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
				...
				} else { printf("Chassis Handle: 0x%04hX\n" ,type2 -> chassis_handle); }
				if (LAZYBIOS_FIELD_STATUS(type2 ,board_type) != LAZYBIOS_FIELD_PRESENT) {
					printf("Board Type: Not Present\n");
				} else { printf("Board Type: %s\n" ,lazybiosType2BoardTypeStr(type2 -> board_type)); }
				if (LAZYBIOS_FIELD_STATUS(type2 ,number_of_contained_object_handles) != LAZYBIOS_FIELD_PRESENT || type2
					-> number_of_contained_object_handles == 0) { } else {
	...
	} else { printf("Failed to get Baseboard information\n\n"); }
}
...
static void printType3 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type3 && ctx -> type3_count > 0) {
		for (size_t i = 0; i < ctx -> type3_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type3 ,type) != LAZYBIOS_FIELD_PRESENT) { printf("Type: Not Present\n"); } else {
				char buf[LAZYBIOS_DECODER_BUF_SIZE];
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,3)) {
				...
				} else { printf("OEM-defined DWORD: 0x%08X\n" ,type3 -> oem_defined); }
				if (LAZYBIOS_FIELD_STATUS(type3 ,height) !=
					LAZYBIOS_FIELD_PRESENT) { printf("Height: Not Present\n"); } else if (type3 -> height == 0x00) {
					printf("Height: Unspecified\n");
				} else { printf("Height: %hhuU\n" ,type3 -> height); }
				if (LAZYBIOS_FIELD_STATUS(type3 ,number_of_power_cords) != LAZYBIOS_FIELD_PRESENT) {
					printf("Number of Power Cords: Not Present\n");
				} else { printf("Number of Power Cords: %hhu\n" ,type3 -> number_of_power_cords); }
				if (LAZYBIOS_FIELD_STATUS(type3 ,contained_element_count) != LAZYBIOS_FIELD_PRESENT) {
					printf("Contained Elements: Not Present\n");
				} else if (type3 -> contained_element_count == 0) { printf("Contained Elements: None\n"); } else if (!
					type3 -> contained_elements) { printf("Contained Elements: Not Present\n"); } else {
					...
					for (uint8_t element_index = 0; element_index < type3 -> contained_element_count; element_index++) {
						uint8_t *record = type3 -> contained_elements + element_index * type3 ->
				...
				if (LAZYBIOS_FIELD_STATUS(type3 ,contained_element_record_length) != LAZYBIOS_FIELD_PRESENT) { } else if
				(type3 -> contained_element_record_length > 0) {
	...
	} else { printf("Failed to get Chassis Information!\n\n"); }
}
...
static void printType4 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type4 && ctx -> type4_count > 0) {
		for (size_t i = 0; i < ctx -> type4_count; i++) {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,5)) {
				...
				if (type4 -> core_count == 0xFF && (LAZYBIOS_FIELD_STATUS(type4 ,core_count_2) ==
					LAZYBIOS_FIELD_PRESENT)) {
					actual_core_count = type4 -> core_count_2;
					has_core_count = 1;
				} else if (LAZYBIOS_FIELD_STATUS(type4 ,core_count) == LAZYBIOS_FIELD_PRESENT && type4 -> core_count !=
					0xFF) {
					actual_core_count = type4 -> core_count;
					has_core_count = 1;
				}
				if (!has_core_count) { printf("Core Count: Not Present\n"); } else {
					printf("Core Count: %u\n" ,actual_core_count);
				}
				uint32_t actual_core_enabled = 0;
				int has_core_enabled = 0;
				if (type4 -> core_enabled == 0xFF && (LAZYBIOS_FIELD_STATUS(type4 ,core_enabled_2) ==
					LAZYBIOS_FIELD_PRESENT)) {
					actual_core_enabled = type4 -> core_enabled_2;
					has_core_enabled = 1;
				} else if (LAZYBIOS_FIELD_STATUS(type4 ,core_enabled) == LAZYBIOS_FIELD_PRESENT && type4 -> core_enabled
					!= 0xFF) {
					actual_core_enabled = type4 -> core_enabled;
					has_core_enabled = 1;
				}
				if (!has_core_enabled) { printf("Core Enabled: Not Present\n"); } else {
					printf("Core Enabled: %u\n" ,actual_core_enabled);
				}
				uint32_t actual_thread_count = 0;
				int has_thread_count = 0;
				if (type4 -> thread_count == 0xFF && (LAZYBIOS_FIELD_STATUS(type4 ,thread_count_2) ==
					LAZYBIOS_FIELD_PRESENT)) {
					actual_thread_count = type4 -> thread_count_2;
					has_thread_count = 1;
				} else if (LAZYBIOS_FIELD_STATUS(type4 ,thread_count) == LAZYBIOS_FIELD_PRESENT && type4 -> thread_count
					!= 0xFF) {
					actual_thread_count = type4 -> thread_count;
					has_thread_count = 1;
				}
				if (!has_thread_count) { printf("Thread Count: Not Present\n"); } else {
	...
	} else { printf("Failed to get Processor information!\n\n"); }
}
...
static void printType5 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type5 && ctx -> type5_count > 0) {
		for (size_t i = 0; i < ctx -> type5_count; i++) {
			...
			char buf[LAZYBIOS_DECODER_BUF_SIZE];
			if (ctx -> type5_count > 1)
				printf("--- Memory Controller %zu ---\n" ,i + 1);
			...
			} else { printf("Maximum Memory Module Size: 2^%hhu MiB\n" ,type5 -> maximum_memory_module_size); }
			...
			} else { printf("Number of Associated Memory Slots: %hhu\n" ,type5 -> number_of_associated_memory_slots); }
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
				if (LAZYBIOS_FIELD_STATUS(type5 ,enabled_error_correcting_capabilities) != LAZYBIOS_FIELD_PRESENT) {
					...
					lazybiosType5ErrorCorrectingCapabilityStr(type5 -> enabled_error_correcting_capabilities ,buf ,
					                                          sizeof(buf));
					printf("Enabled Error Correcting Capabilities: %s\n" ,buf);
				}
			} else if (!compact_output) { printf("Enabled Error Correcting Capabilities: [SMBIOS 2.1 required]\n"); }
			printf("\n");
		}
	} else { printf("Failed to get Memory Controller information\n\n"); }
}
...
static void printType6 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type6 && ctx -> type6_count > 0) {
		for (size_t i = 0; i < ctx -> type6_count; i++) {
			...
			char buf[LAZYBIOS_DECODER_BUF_SIZE];
			if (ctx -> type6_count > 1)
				printf("--- Memory Module %zu ---\n" ,i + 1);
			printf("Socket Designation: %s\n" ,
			...
			if (LAZYBIOS_FIELD_STATUS(type6 ,current_speed) != LAZYBIOS_FIELD_PRESENT) {
				printf("Current Speed: Not Present\n");
			} else if (type6 -> current_speed == 0) { printf("Current Speed: Unknown\n"); } else {
	...
	} else { printf("Failed to get Memory Module information\n\n"); }
}
...
static void printType7 (lazybiosCTX_t *ctx) {
		...
		for (size_t i = 0; i < ctx -> type7_count; i++) {
			...
			if (type7 -> maximum_cache_size == 0xFFFF) {
				if (LAZYBIOS_FIELD_STATUS(type7 ,maximum_cache_size_2) != LAZYBIOS_FIELD_PRESENT) {
					...
					if (size_kb > 1024) { printf("Maximum Cache Size: %.2f MB\n" ,(double)size_kb / 1024.0); } else {
			...
			if (type7 -> installed_size == 0xFFFF) {
				if (LAZYBIOS_FIELD_STATUS(type7 ,installed_cache_size_2) != LAZYBIOS_FIELD_PRESENT) {
					...
					if (size_kb > 1024) { printf("Installed Size: %.2f MB\n" ,(double)size_kb / 1024.0); } else {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
				...
				} else if (type7 -> cache_speed == 0) { printf("Cache Speed: Unknown\n"); } else {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,3 ,1)) {
				if (LAZYBIOS_FIELD_STATUS(type7 ,maximum_cache_size_2) != LAZYBIOS_FIELD_PRESENT) {
					...
					uint64_t size_kb = lazybiosType7CacheU32(type7 -> maximum_cache_size_2);
					if (size_kb > 1024) { printf("Maximum Cache Size 2: %.2f MB\n" ,(double)size_kb / 1024.0); } else {
				...
				if (LAZYBIOS_FIELD_STATUS(type7 ,installed_cache_size_2) != LAZYBIOS_FIELD_PRESENT) {
					...
					uint64_t size_kb = lazybiosType7CacheU32(type7 -> installed_cache_size_2);
					if (size_kb >
						1024) { printf("Installed Cache Size 2: %.2f MB\n" ,(double)size_kb / 1024.0); } else {
	...
	} else { printf("Failed to get Cache information\n\n"); }
}
...
static void printType8 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get Port Connector information\n\n"); }
}
	...
	if (ctx -> Type9 && ctx -> type9_count > 0) {
		for (size_t i = 0; i < ctx -> type9_count; i++) {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
				...
				if (!compact_output)
					printf("Slot Characteristics 2: [SMBIOS 2.1 required]\n");
			}
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,3 ,2)) {
				...
				if (!compact_output)
					printf("Data Bus Width: [SMBIOS 3.2 required]\n");
				if (!compact_output)
					printf("Peer Groups: [SMBIOS 3.2 required]\n");
			}
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,3 ,5)) {
				...
				} else { printf("Slot Height: %s\n" ,lazybiosType9SlotHeightStr(type9 -> slot_height)); }
			} else {
				if (!compact_output)
					printf("Slot Height: [SMBIOS 3.5 required]\n");
			}
			printf("\n");
		}
	} else { printf("Failed to get System Slot information\n\n"); }
}
...
static void printType10 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type10 && ctx -> type10_count > 0) {
		for (size_t i = 0; i < ctx -> type10_count; i++) {
			...
			if (ctx -> type10_count > 1)
				printf("--- On Board Devices Structure %zu ---\n" ,i + 1);
	...
	} else { printf("Failed to get On Board Devices information\n\n"); }
}
...
static void printType11 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get OEM Strings information\n\n"); }
}
...
static void printType12 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get System Configuration Options information\n\n"); }
}
...
static void printType13 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get Firmware Language information\n\n"); }
}
...
static void printType14 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type14 && ctx -> type14_count > 0) {
		for (size_t i = 0; i < ctx -> type14_count; i++) {
			...
			} else { printf("Item Count: %zu\n" ,type14 -> item_count); }
			if (LAZYBIOS_FIELD_STATUS(type14 ,items) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Items: Not Present\n"); } else {
	...
	} else { printf("Failed to get Group Associations information\n\n"); }
}
...
static void printType15 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type15 && ctx -> type15_count > 0) {
		for (size_t i = 0; i < ctx -> type15_count; i++) {
			...
			} else if (type15 -> log_change_token == 0) { printf("Log Change Token: Not Implemented\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type15 ,access_method_address) != LAZYBIOS_FIELD_PRESENT) {
			...
			} else if (LAZYBIOS_FIELD_STATUS(type15 ,access_method) == LAZYBIOS_FIELD_PRESENT && type15 -> access_method
				<= 0x02) {
				...
				== 0x03) { printf("Physical Address: 0x%08X\n" ,type15 -> access_method_address); } else if (
				LAZYBIOS_FIELD_STATUS(type15 ,access_method) == LAZYBIOS_FIELD_PRESENT && type15 -> access_method ==
				0x04) {
				printf("GPNV Handle: 0x%04hX\n" ,lazybiosType15GPNVHandle(type15 -> access_method_address));
			} else { printf("Access Method Address: 0x%08X\n" ,type15 -> access_method_address); }
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
	...
	} else { printf("Failed to get System Event Log information\n\n"); }
}
...
static void printType16 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type16 && ctx -> type16_count > 0) {
		for (size_t i = 0; i < ctx -> type16_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type16 ,use) != LAZYBIOS_FIELD_PRESENT) { printf("Use: Not Present\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type16 ,maximum_capacity) != LAZYBIOS_FIELD_PRESENT) {
				...
				uint64_t capacity = lazybiosType16MaximumCapacityBytes(type16 -> maximum_capacity ,
				                                                       type16 -> extended_maximum_capacity);
				...
				} else { printf("Maximum Capacity: %llu bytes\n" ,(unsigned long long)capacity); }
			}
			if (type16 -> memory_error_information_handle ==
				0xFFFE) { printf("Memory Error Information Handle: Not Provided\n"); } else if (type16 ->
				memory_error_information_handle ==
				0xFFFF) { printf("Memory Error Information Handle: No Error Detected\n"); } else if (
				LAZYBIOS_FIELD_STATUS(type16 ,memory_error_information_handle) != LAZYBIOS_FIELD_PRESENT) {
				printf("Memory Error Information Handle: Not Present\n");
			} else { printf("Memory Error Information Handle: 0x%04hX\n" ,type16 -> memory_error_information_handle); }
			...
			} else { printf("Number of Memory Devices: %hu\n" ,type16 -> number_of_memory_devices); }
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,7)) {
			...
			} else if (!compact_output) { printf("Extended Maximum Capacity: [SMBIOS 2.7 required]\n"); }
			printf("\n");
		}
	} else { printf("Failed to get Physical Memory Array information\n\n"); }
}
...
static void printType17 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type17 && ctx -> type17_count > 0) {
		for (size_t i = 0; i < ctx -> type17_count; i++) {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,1)) {
				...
				} else { printf("Physical Memory Array Handle: 0x%04hX\n" ,type17 -> physical_memory_array_handle); }
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,3)) {
				if (LAZYBIOS_FIELD_STATUS(type17 ,speed) !=
					LAZYBIOS_FIELD_PRESENT) { printf("Speed: Not Present\n"); } else {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,6)) {
				...
				} else { printf("Attributes: %u rank(s)\n" ,type17 -> attributes & 0x7F); }
			} else {
				if (!compact_output)
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,3 ,2)) {
				...
				if (LAZYBIOS_FIELD_STATUS(type17 ,memory_operating_mode_capability) != LAZYBIOS_FIELD_PRESENT) {
					...
					lazybiosType17OperatingModeCapabilityStr(type17 -> memory_operating_mode_capability ,buf ,
					                                         sizeof(buf));
				...
				if (LAZYBIOS_FIELD_STATUS(type17 ,memory_subsystem_controller_manufacturer_id) !=
					LAZYBIOS_FIELD_PRESENT) {
					printf("Memory Subsystem Controller Manufacturer ID: Not Present\n");
				} else {
					lazybiosType17ModuleManufacturerIDStr(type17 -> memory_subsystem_controller_manufacturer_id ,buf ,
					                                      sizeof(buf));
				...
				if (LAZYBIOS_FIELD_STATUS(type17 ,memory_subsystem_controller_product_id) != LAZYBIOS_FIELD_PRESENT) {
					...
					lazybiosType17ModuleManufacturerIDStr(type17 -> memory_subsystem_controller_product_id ,buf ,
					                                      sizeof(buf));
	...
	} else { printf("Failed to get Memory Device information\n\n"); }
}
...
static void printType18 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type18 && ctx -> type18_count > 0) {
		for (size_t i = 0; i < ctx -> type18_count; i++) {
			...
			} else if (type18 -> error_resolution == 0x80000000U) { printf("Error Resolution: Unknown\n"); } else {
				printf("Error Resolution: %u bytes\n" ,type18 -> error_resolution);
			}
			printf("\n");
		}
	} else { printf("Failed to get 32-Bit Memory Error information\n\n"); }
}
...
static void printType19 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type19 && ctx -> type19_count > 0) {
		for (size_t i = 0; i < ctx -> type19_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type19 ,starting_address) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Starting Address: Not Present\n"); } else if (type19 ->
				starting_address == 0xFFFFFFFFU && LAZYBIOS_FIELD_STATUS(type19 ,extended_starting_address) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Starting Address: Not Present\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type19 ,ending_address) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Ending Address: Not Present\n"); } else if (type19 -> ending_address
				== 0xFFFFFFFFU && LAZYBIOS_FIELD_STATUS(type19 ,extended_ending_address) != LAZYBIOS_FIELD_PRESENT) {
			...
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,7)) {
				...
				if (LAZYBIOS_FIELD_STATUS(type19 ,extended_ending_address) != LAZYBIOS_FIELD_PRESENT) {
					...
					0xFFFFFFFFU) { printf("Extended Ending Address: Not Used\n"); } else {
	...
	} else { printf("Failed to get Memory Array Mapped Address information\n\n"); }
}
...
static void printType20 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type20 && ctx -> type20_count > 0) {
		for (size_t i = 0; i < ctx -> type20_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type20 ,starting_address) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Starting Address: Not Present\n"); } else if (type20 ->
				starting_address == 0xFFFFFFFFU && LAZYBIOS_FIELD_STATUS(type20 ,extended_starting_address) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Starting Address: Not Present\n"); } else {
				printf("Starting Address: 0x%016llX\n" ,
				       (unsigned long long)lazybiosType20StartingAddressBytes(
					       type20 -> starting_address ,type20 -> extended_starting_address));
			}
			if (LAZYBIOS_FIELD_STATUS(type20 ,ending_address) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Ending Address: Not Present\n"); } else if (type20 -> ending_address
				== 0xFFFFFFFFU && LAZYBIOS_FIELD_STATUS(type20 ,extended_ending_address) != LAZYBIOS_FIELD_PRESENT) {
				printf("Ending Address: Not Present\n");
			} else {
				printf("Ending Address: 0x%016llX\n" ,
			...
			} else { printf("Memory Device Handle: 0x%04hX\n" ,type20 -> memory_device_handle); }
			...
			} else if (type20 -> partition_row_position == 0xFF) { printf("Partition Row Position: Unknown\n"); } else
				if (type20 -> partition_row_position == 0) { printf("Partition Row Position: Reserved\n"); } else {
			...
			} else if (type20 -> interleave_position == 0xFF) { printf("Interleave Position: Unknown\n"); } else if (
				type20 -> interleave_position == 0) { printf("Interleave Position: Non-interleaved\n"); } else {
			...
			} else if (type20 -> interleaved_data_depth == 0xFF) { printf("Interleaved Data Depth: Unknown\n"); } else
				if (type20 -> interleaved_data_depth ==
					0) { printf("Interleaved Data Depth: Not Interleaved\n"); } else {
	...
	} else { printf("Failed to get Memory Device Mapped Address information\n\n"); }
}
...
static void printType21 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type21 && ctx -> type21_count > 0) {
		for (size_t i = 0; i < ctx -> type21_count; i++) {
			...
			} else { printf("Number of Buttons: %hhu\n" ,type21 -> number_of_buttons); }
			printf("\n");
		}
	} else { printf("Failed to get Built-in Pointing Device information\n\n"); }
}
...
static void printType22 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type22 && ctx -> type22_count > 0) {
		for (size_t i = 0; i < ctx -> type22_count; i++) {
			...
			} else { printf("Device Chemistry: %s\n" ,lazybiosType22DeviceChemistryStr(type22 -> device_chemistry)); }
			if (LAZYBIOS_FIELD_STATUS(type22 ,design_capacity) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Design Capacity: Not Present\n"); } else if (type22 -> design_capacity
				== 0) { printf("Design Capacity: Unknown\n"); } else if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,2) &&
				...
				uint8_t multiplier = lazybiosIsVersionPlus(ctx -> DMIData ,2 ,2) ? type22 -> design_capacity_multiplier
					                     : 1;
			...
			} else if (type22 -> design_voltage == 0) { printf("Design Voltage: Unknown\n"); } else {
			...
			} else if (type22 -> maximum_error == 0xFF) { printf("Maximum Error in Battery Data: Unknown\n"); } else {
				printf("Maximum Error in Battery Data: %hhu%%\n" ,type22 -> maximum_error);
			}
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,2)) {
				if (type22 -> serial_number) { printf("SBDS Serial Number: Not Used\n"); } else if (
					LAZYBIOS_FIELD_STATUS(type22 ,sbds_serial_number) != LAZYBIOS_FIELD_PRESENT) {
					printf("SBDS Serial Number: Not Present\n");
				} else { printf("SBDS Serial Number: 0x%04hX\n" ,type22 -> sbds_serial_number); }
				if (type22 -> manufacture_date) { printf("SBDS Manufacture Date: Not Used\n"); } else if (
					LAZYBIOS_FIELD_STATUS(type22 ,sbds_manufacture_date) != LAZYBIOS_FIELD_PRESENT) {
				...
				if (type22 -> device_chemistry != 0x02) { printf("SBDS Device Chemistry: Not Used\n"); } else if (
					LAZYBIOS_FIELD_STATUS(type22 ,sbds_device_chemistry) != LAZYBIOS_FIELD_PRESENT) {
	...
	} else { printf("Failed to get Portable Battery information\n\n"); }
}
...
static void printType23 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type23 && ctx -> type23_count > 0) {
		for (size_t i = 0; i < ctx -> type23_count; i++) {
			...
			} else if (type23 -> reset_count == 0xFFFF) { printf("Reset Count: Unknown\n"); } else {
				printf("Reset Count: %hu\n" ,type23 -> reset_count);
			}
			if (LAZYBIOS_FIELD_STATUS(type23 ,reset_limit) != LAZYBIOS_FIELD_PRESENT) {
				printf("Reset Limit: Not Present\n");
			} else if (type23 -> reset_limit == 0xFFFF) { printf("Reset Limit: Unknown\n"); } else {
			...
			} else if (type23 -> timer_interval == 0xFFFF) { printf("Timer Interval: Unknown\n"); } else {
				printf("Timer Interval: %hu minutes\n" ,type23 -> timer_interval);
			}
			if (LAZYBIOS_FIELD_STATUS(type23 ,timeout) !=
				LAZYBIOS_FIELD_PRESENT) { printf("Timeout: Not Present\n"); } else if (type23 -> timeout == 0xFFFF) {
				printf("Timeout: Unknown\n");
			} else { printf("Timeout: %hu minutes\n" ,type23 -> timeout); }
			printf("\n");
		}
	} else { printf("Failed to get System Reset information\n\n"); }
}
...
static void printType24 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get Hardware Security information\n\n"); }
}
...
static void printType25 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get System Power Controls information\n\n"); }
}
...
static void printType26 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type26 && ctx -> type26_count > 0) {
		for (size_t i = 0; i < ctx -> type26_count; i++) {
			...
			} else if (type26 -> maximum_value == 0x8000) { printf("Maximum Value: Unknown\n"); } else {
			...
			} else if (type26 -> minimum_value == 0x8000) { printf("Minimum Value: Unknown\n"); } else {
			...
			} else if (type26 -> resolution == 0x8000) { printf("Resolution: Unknown\n"); } else {
			...
			} else { printf("OEM-defined Information: 0x%08X\n" ,type26 -> oem_defined); }
			if (LAZYBIOS_FIELD_STATUS(type26 ,nominal_value) != LAZYBIOS_FIELD_PRESENT) {
				printf("Nominal Value: Not Present\n");
			} else if (type26 -> nominal_value == 0x8000) { printf("Nominal Value: Unknown\n"); } else {
				printf("Nominal Value: %hu mV\n" ,type26 -> nominal_value);
			}
			printf("\n");
		}
	} else { printf("Failed to get Voltage Probe information\n\n"); }
}
...
static void printType27 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type27 && ctx -> type27_count > 0) {
		for (size_t i = 0; i < ctx -> type27_count; i++) {
			...
			} else { printf("Temperature Probe Handle: 0x%04hX\n" ,type27 -> temperature_probe_handle); }
			...
			} else if (type27 -> cooling_unit_group == 0) { printf("Cooling Unit Group: None\n"); } else {
			...
			} else if (type27 -> nominal_speed == 0x8000) { printf("Nominal Speed: Unknown or Non-rotating\n"); } else {
				printf("Nominal Speed: %hu rpm\n" ,type27 -> nominal_speed);
			}
			if (lazybiosIsVersionPlus(ctx -> DMIData ,2 ,7)) {
				printf("Description: %s\n" ,type27 -> description ? type27 -> description : "Not Present");
			} else if (!compact_output) { printf("Description: [SMBIOS 2.7 required]\n"); }
			printf("\n");
		}
	} else { printf("Failed to get Cooling Device information\n\n"); }
}
	...
	if (ctx -> Type28 && ctx -> type28_count > 0) {
		for (size_t i = 0; i < ctx -> type28_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type28 ,maximum_value) != LAZYBIOS_FIELD_PRESENT) {
				printf("Maximum Value: Not Present\n");
			} else if (type28 -> maximum_value == 0x8000) { printf("Maximum Value: Unknown\n"); } else {
				printf("Maximum Value: %hu.%hu degrees C\n" ,type28 -> maximum_value / 10 ,
				       type28 -> maximum_value % 10);
			}
			if (LAZYBIOS_FIELD_STATUS(type28 ,minimum_value) != LAZYBIOS_FIELD_PRESENT) {
				printf("Minimum Value: Not Present\n");
			} else if (type28 -> minimum_value == INT16_MIN) { printf("Minimum Value: Unknown\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type28 ,resolution) != LAZYBIOS_FIELD_PRESENT) {
				printf("Resolution: Not Present\n");
			} else if (type28 -> resolution == 0x8000) { printf("Resolution: Unknown\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type28 ,tolerance) != LAZYBIOS_FIELD_PRESENT) {
				printf("Tolerance: Not Present\n");
			} else if (type28 -> tolerance == 0x8000) { printf("Tolerance: Unknown\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type28 ,accuracy) != LAZYBIOS_FIELD_PRESENT) {
				printf("Accuracy: Not Present\n");
			} else if (type28 -> accuracy == 0x8000) { printf("Accuracy: Unknown\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type28 ,nominal_value) != LAZYBIOS_FIELD_PRESENT) {
				printf("Nominal Value: Not Present\n");
			} else if (type28 -> nominal_value == 0x8000) { printf("Nominal Value: Unknown\n"); } else {
				printf("Nominal Value: %hu.%hu degrees C\n" ,type28 -> nominal_value / 10 ,
				       type28 -> nominal_value % 10);
			}
			printf("\n");
		}
	} else { printf("Failed to get Temperature Probe information\n\n"); }
}
	...
	if (ctx -> Type29 && ctx -> type29_count > 0) {
		for (size_t i = 0; i < ctx -> type29_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type29 ,tolerance) != LAZYBIOS_FIELD_PRESENT) {
				printf("Tolerance: Not Present\n");
			} else if (type29 -> tolerance == 0x8000) { printf("Tolerance: Unknown\n"); } else {
				printf("Tolerance: +/- %hu mA\n" ,type29 -> tolerance);
			}
			if (LAZYBIOS_FIELD_STATUS(type29 ,accuracy) != LAZYBIOS_FIELD_PRESENT) {
				printf("Accuracy: Not Present\n");
			} else if (type29 -> accuracy == 0x8000) { printf("Accuracy: Unknown\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type29 ,nominal_value) != LAZYBIOS_FIELD_PRESENT) {
				printf("Nominal Value: Not Present\n");
			} else if (type29 -> nominal_value == 0x8000) { printf("Nominal Value: Unknown\n"); } else {
				printf("Nominal Value: %hu mA\n" ,type29 -> nominal_value);
			}
			printf("\n");
		}
	} else { printf("Failed to get Electrical Current Probe information\n\n"); }
}
...
static void printType30 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get Out-of-Band Remote Access information\n\n"); }
}
		...
		for (size_t i = 0; i < ctx -> type31_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type31 ,reserved_1) != LAZYBIOS_FIELD_PRESENT ||
				...
				printf("Reserved Fields: 0x%02X, 0x%04hX, 0x%016llX, 0x%08X\n" ,type31 -> reserved_1 ,
				       type31 -> reserved_2 ,(unsigned long long)type31 -> reserved_3 ,type31 -> reserved_4);
			}
			printf("\n");
		}
	} else { printf("Failed to get Boot Integrity Services Entry Point information\n\n"); }
}
...
static void printType32 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get System Boot Information\n\n"); }
}
	...
	if (ctx -> Type33 && ctx -> type33_count > 0) {
		for (size_t i = 0; i < ctx -> type33_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type33 ,vendor_syndrome) != LAZYBIOS_FIELD_PRESENT) {
				printf("Vendor Syndrome: Not Present\n");
			} else if (type33 -> vendor_syndrome == 0) { printf("Vendor Syndrome: Unknown\n"); } else {
			...
			} else { printf("Device Error Address: 0x%016llX\n" ,(unsigned long long)type33 -> device_error_address); }
			if (LAZYBIOS_FIELD_STATUS(type33 ,error_resolution) != LAZYBIOS_FIELD_PRESENT) {
				printf("Error Resolution: Not Present\n");
			} else if (type33 -> error_resolution == 0x80000000U) { printf("Error Resolution: Unknown\n"); } else {
				printf("Error Resolution: %u bytes\n" ,type33 -> error_resolution);
			}
			printf("\n");
		}
	} else { printf("Failed to get 64-Bit Memory Error information\n\n"); }
}
...
static void printType36 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type36 && ctx -> type36_count > 0) {
		for (size_t i = 0; i < ctx -> type36_count; i++) {
			...
			} else { printf("Lower Non-critical Threshold: %hu\n" ,type36 -> lower_threshold_non_critical); }
			...
			} else { printf("Upper Non-critical Threshold: %hu\n" ,type36 -> upper_threshold_non_critical); }
			if (type36 -> lower_threshold_critical ==
				0x8000) { printf("Lower Critical Threshold: Unavailable\n"); } else if (
				LAZYBIOS_FIELD_STATUS(type36 ,lower_threshold_critical) != LAZYBIOS_FIELD_PRESENT) {
				printf("Lower Critical Threshold: Not Present\n");
			} else { printf("Lower Critical Threshold: %hu\n" ,type36 -> lower_threshold_critical); }
			if (type36 -> upper_threshold_critical ==
				0x8000) { printf("Upper Critical Threshold: Unavailable\n"); } else if (
				LAZYBIOS_FIELD_STATUS(type36 ,upper_threshold_critical) != LAZYBIOS_FIELD_PRESENT) {
				printf("Upper Critical Threshold: Not Present\n");
			} else { printf("Upper Critical Threshold: %hu\n" ,type36 -> upper_threshold_critical); }
			...
			} else { printf("Lower Non-recoverable Threshold: %hu\n" ,type36 -> lower_threshold_non_recoverable); }
			...
			} else { printf("Upper Non-recoverable Threshold: %hu\n" ,type36 -> upper_threshold_non_recoverable); }
			printf("\n");
		}
	} else { printf("Failed to get Management Device Threshold Data information\n\n"); }
}
...
static void printType37 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get Memory Channel information\n\n"); }
}
...
static void printType38 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type38 && ctx -> type38_count > 0) {
		for (size_t i = 0; i < ctx -> type38_count; i++) {
			...
			} else { printf("I2C Target Address: 0x%02hhX\n" ,type38 -> i2c_target_address); }
			...
			} else { printf("NV Storage Device Address: 0x%02hhX\n" ,type38 -> nv_storage_device_address); }
			if (LAZYBIOS_FIELD_STATUS(type38 ,base_address) != LAZYBIOS_FIELD_PRESENT) {
				...
				} else { printf("Base Address: 0x%016llX (raw)\n" ,(unsigned long long)type38 -> base_address); }
			}
			if (LAZYBIOS_FIELD_STATUS(type38 ,base_address_modifier_interrupt_info) != LAZYBIOS_FIELD_PRESENT) {
			...
			} else { printf("Interrupt Number: %hhu\n" ,type38 -> interrupt_number); }
			printf("\n");
		}
	} else { printf("Failed to get IPMI Device information\n\n"); }
}
...
static void printType39 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type39 && ctx -> type39_count > 0) {
		for (size_t i = 0; i < ctx -> type39_count; i++) {
			...
			} else if (type39 -> power_unit_group == 0) { printf("Power Unit Group: 0 (Not Redundant)\n"); } else {
			...
			} else if (type39 -> max_power_capacity == 0x8000) { printf("Maximum Power Capacity: Unknown\n"); } else {
			...
			} else { printf("Input Current Probe Handle: 0x%04hX\n" ,type39 -> input_current_probe_handle); }
			printf("\n");
		}
	} else { printf("Failed to get System Power Supply information\n\n"); }
}
...
static void printType40 (lazybiosCTX_t *ctx) {
		...
		for (size_t i = 0; i < ctx -> type40_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type40 ,additional_information_entries) != LAZYBIOS_FIELD_PRESENT) {
				...
				for (size_t j = 0; j < type40 -> additional_information_entry_count; j++) {
					...
					if (entry -> field_status . string !=
						LAZYBIOS_FIELD_PRESENT) { printf("String: Not Present\n"); } else {
	...
	} else { printf("Failed to get Additional Information\n\n"); }
}
...
static void printType41 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type41 && ctx -> type41_count > 0) {
		for (size_t i = 0; i < ctx -> type41_count; i++) {
			...
			} else { printf("Device Type Instance: %hhu\n" ,type41 -> device_type_instance); }
			...
			} else if (type41 -> bus_number == 0xFF) { printf("Bus Number: Not Applicable\n"); } else {
	...
	} else { printf("Failed to get Onboard Devices Extended information\n\n"); }
}
...
static void printType42 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type42 && ctx -> type42_count > 0) {
		for (size_t i = 0; i < ctx -> type42_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type42 ,interface_type) != LAZYBIOS_FIELD_PRESENT) {
				...
				printf("Interface Type: %s (0x%02hhX)\n" ,lazybiosType42InterfaceTypeStr(type42 -> interface_type) ,
				       type42 -> interface_type);
			}
			...
			} else { printf("Protocol Record Count: %hhu\n" ,type42 -> number_of_protocol_records); }
			if (LAZYBIOS_FIELD_STATUS(type42 ,protocol_records) != LAZYBIOS_FIELD_PRESENT) {
				...
				for (size_t j = 0; j < type42 -> number_of_protocol_records; j++) {
					...
					printf("Protocol Type: %s (0x%02hhX)\n" ,lazybiosType42ProtocolTypeStr(protocol -> protocol_type) ,
	...
	} else { printf("Failed to get Management Controller Host Interface information\n\n"); }
}
	...
	if (ctx -> Type43 && ctx -> type43_count > 0) {
		for (size_t i = 0; i < ctx -> type43_count; i++) {
			...
			if (LAZYBIOS_FIELD_STATUS(type43 ,major_spec_version) != LAZYBIOS_FIELD_PRESENT ||
				...
				printf("TPM Specification Version: %hhu.%hhu\n" ,type43 -> major_spec_version ,
				       type43 -> minor_spec_version);
			}
			...
			if (LAZYBIOS_FIELD_STATUS(type43 ,oem_defined) != LAZYBIOS_FIELD_PRESENT) {
				printf("OEM-defined: Not Present\n");
			} else { printf("OEM-defined: 0x%08X\n" ,type43 -> oem_defined); }
			printf("\n");
		}
	} else { printf("Failed to get TPM Device information\n\n"); }
}
...
static void printType44 (lazybiosCTX_t *ctx) {
	...
	} else { printf("Failed to get Processor Additional Information\n\n"); }
}
...
static void printType45 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type45 && ctx -> type45_count > 0) {
		for (size_t i = 0; i < ctx -> type45_count; i++) {
			...
			printf("Lowest Supported Firmware Version: %s\n" ,
			       type45 -> lowest_supported_firmware_version ? type45 -> lowest_supported_firmware_version
				       : "Not Present");
			if (LAZYBIOS_FIELD_STATUS(type45 ,image_size) != LAZYBIOS_FIELD_PRESENT) {
				printf("Image Size: Not Present\n");
			} else if (type45 -> image_size == UINT64_MAX) { printf("Image Size: Unknown\n"); } else {
			...
			if (LAZYBIOS_FIELD_STATUS(type45 ,state) !=
				LAZYBIOS_FIELD_PRESENT) { printf("State: Not Present\n"); } else {
			...
			} else { printf("Number of Associated Components: %hhu\n" ,type45 -> number_of_associated_components); }
	...
	} else { printf("Failed to get Firmware Inventory information\n\n"); }
}
...
static void printType46 (lazybiosCTX_t *ctx) {
	...
	if (ctx -> Type46 && ctx -> type46_count > 0) {
		for (size_t i = 0; i < ctx -> type46_count; i++) {
			...
			} else { printf("Parent Handle: 0x%04hX\n" ,type46 -> parent_handle); }
			printf("\n");
		}
	} else { printf("Failed to get String Property information\n\n"); }
}
...
int print_smbios_version_info (lazybiosCTX_t *ctx) {
	...
	if (ctx -> DMIData -> entry_tag == SMBIOS_VER_3X) {
		...
		printf("Anchor: %c%c%c%c%c\n" ,v3 -> anchor[0] ,v3 -> anchor[1] ,v3 -> anchor[2] ,v3 -> anchor[3] ,
		       v3 -> anchor[4]);
		...
		if (ctx -> backend ==
			LAZYBIOS_BACKEND_WINDOWS) { printf("Table Address: Not available (Windows API)\n"); } else {
		...
		printf("Intermediate Anchor: %c%c%c%c%c\n" ,v2 -> intermediate_anchor[0] ,v2 -> intermediate_anchor[1] ,
		       v2 -> intermediate_anchor[2] ,v2 -> intermediate_anchor[3] ,v2 -> intermediate_anchor[4]);
...
static inline void print_usage (const char *progname) {
	...
	printf(
		"  --type <number>             prints the SMBIOS type <number>, so --type 0 means Firmware Information etc.\n");
	printf("  --compact                   Hide fields that require a newer SMBIOS version\n");
	printf(
		"  --dump <dir_to_dump>        Dumps both the raw entry info and raw DMI table into 2 files into <dir_to_dump> directory, If OS is Windows it only dumps DMI.bin\n If <dir_to_dump> isn't specified it will default to the current directory\n");
	printf("  --sources <entry> <dmi>     Usees the parser on the 2 specified files <entry> and <dmi>\n");
	printf(
		"  --single-source <binary>    Uses the parser on the single specified file <binary>, which should hold the entry point first and dmi data merged together\n");
}
...
int main (int argc, const char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i] ,"--help") == 0) {
			...
			lazybiosCleanup(ctx);
			return 0;
		} else if (strcmp(argv[i] ,"--compact") == 0) { compact_output = 1; } else if (strcmp(argv[i] ,"--type") == 0) {
			if (i + 1 < argc) {
	...
	if (dump_dir) {
		...
		snprintf(path_dmi ,sizeof(path_dmi) ,"%s\\DMI.bin" ,dump_dir); snprintf(
			path_entry ,sizeof(path_entry) ,"%s\\generated_smbios_entry_point.bin" ,dump_dir);
		#else
	...
	if (lazybiosCleanup(ctx) == 0) { printf("Library cleanup completed!\n"); } else {
